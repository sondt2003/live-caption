...
class AudioSelector(QWidget):
    ...
    def __init__(self, label, parent=None):
        ...
        self.browse_button = QPushButton(i18n.get('browse'))
    ...
    def browse_file(self):
        file_path, _ = QFileDialog.getOpenFileName(self, i18n.get('select_local_video'), "", "音频文件 (*.mp3 *.wav *.ogg)")
        if file_path:
...
class VideoPlayer(QWidget):
    ...
    def __init__(self, label, parent=None):
        ...
        self.volume_slider.setToolTip(i18n.get('volume'))
        ...
        self.play_button = QPushButton(i18n.get('play'))
        self.play_button.clicked.connect(self.play_pause)

        # 添加暂停和停止按钮
        self.stop_button = QPushButton(i18n.get('stop'))
        ...
        volume_layout.addWidget(QLabel(i18n.get('volume') + ":"))
    ...
    def set_volume(self, volume):
        ...
        self.status_label.setText(f"{i18n.get('volume')}: {volume}%")

    def set_video(self, path):
        """设置视频源"""
        if not os.path.exists(path):
            self.status_label.setText(f"{i18n.get('error_file_not_exist')}")
            return
        ...
        try:
            ...
            self.status_label.setText(f"{i18n.get('loaded')}: {os.path.basename(path)}")
    ...
    def play_pause(self):
        ...
        if not self.video_path:
            self.status_label.setText(i18n.get('error_no_video_loaded'))
            return

        if self.media_player.playbackState() == QMediaPlayer.PlayingState:
            self.media_player.pause()
            self.play_button.setText(i18n.get('pause'))
            self.status_label.setText(i18n.get('playing'))
        else:
            self.media_player.play()
            self.play_button.setText(i18n.get('play'))
            self.status_label.setText(i18n.get('paused'))

    def stop_video(self):
        """停止视频播放"""
        self.media_player.stop()
        self.play_button.setText(i18n.get('play'))
        self.status_label.setText(i18n.get('stopped'))

    def handle_error(self, error, error_string):
        """处理媒体播放器错误"""
        self.status_label.setText(f"{i18n.get('play_error')}: {error_string}")
