...
def load_model(model_path="tts_models/multilingual/multi-dataset/xtts_v2", device='auto'):
    ...
    t_start = time.time()
    try:
        # Check if it is a local path or model name
        if os.path.exists(model_path):
             model = TTS(model_path=model_path, config_path=os.path.join(model_path, 'config.json')).to(device)
        else:
             model = TTS(model_path).to(device)
    except Exception as e:
        logger.warning(f"Failed to load model from {model_path}: {e}")
        # Fallback to standard model name if path fails
...
def tts(text, output_path, speaker_wav, model_name="tts_models/multilingual/multi-dataset/xtts_v2", device='auto', target_language='中文'):
    ...
    elif target_language in language_map.values():
        language = target_language
    else:
        # Fallback or strict?
        # XTTS logic
        if target_language == 'vi' or target_language == 'Vietnamese':
             logger.warning("XTTS does not support Vietnamese. Using English as fallback.")
             language = 'en'
        else:
             language = 'en'
