...
from .step045_tts_vits import tts as vits_tts
...
def adjust_audio_length(wav_path, desired_length, sample_rate = 24000, min_speed_factor = 0.6, max_speed_factor = 1.1):
    try:
        if not os.path.exists(wav_path) or os.path.getsize(wav_path) == 0:
            raise FileNotFoundError(f"File not found or empty: {wav_path}")
        wav, sample_rate = librosa.load(wav_path, sr=sample_rate)
    except Exception as e:
        logger.warning(f"Failed to load {wav_path}, trying fallback: {e}")
        if wav_path.endswith('.wav'):
            wav_path_fallback = wav_path.replace('.wav', '.mp3')
        else:
            wav_path_fallback = wav_path.replace('.mp3', '.wav')
        
        try:
            if not os.path.exists(wav_path_fallback) or os.path.getsize(wav_path_fallback) == 0:
                raise FileNotFoundError(f"Fallback file not found or empty: {wav_path_fallback}")
            wav, sample_rate = librosa.load(wav_path_fallback, sr=sample_rate)
        except Exception as e2:
            logger.error(f"Fallback also failed for {wav_path_fallback}: {e2}")
            # Final fallback: return a small silence buffer
...
tts_support_languages = {
    ...
    'bytedance': [],
    'GPTSoVits': [],
    'EdgeTTS': ['中文', 'English', 'Japanese', 'Korean', 'French', 'Polish', 'Spanish', 'Vietnamese', 'Tiếng Việt'],
    # zero_shot usage, <|zh|><|en|><|jp|><|yue|><|ko|> for Chinese/English/Japanese/Cantonese/Korean
    'cosyvoice': ['中文', '粤语', 'English', 'Japanese', 'Korean', 'French'], 
    'vits': ['Vietnamese', 'Tiếng Việt'],
}

def generate_wavs(method, folder, target_language='中文', voice = 'zh-CN-XiaoxiaoNeural'):
    assert method in ['xtts', 'bytedance', 'cosyvoice', 'EdgeTTS', 'vits']
    ...
    for i, line in enumerate(transcript):
        ...
        if method == 'bytedance':
            ...
            vits_tts(text, output_path, speaker_wav=speaker_wav, target_language='vi')
        start = line['start']
        end = line['end']
        length = end-start
        ...
        if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
            logger.error(f'TTS failed to generate file or file is empty: {output_path}')
