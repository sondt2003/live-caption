...
from .utils.step000_video_downloader import get_info_list_from_url, download_single_video, get_target_folder
from .utils.step010_demucs_vr import separate_all_audio_under_folder, init_demucs, release_model
from .asr.step020_asr import transcribe_all_audio_under_folder
from .asr.step021_asr_whisperx import init_whisperx, init_diarize
from .asr.step022_asr_funasr import init_funasr
from .translation.step030_translation import translate_all_transcript_under_folder
from .tts.step040_tts import generate_all_wavs_under_folder
from .tts.step042_tts_xtts import init_TTS
from .tts.step043_tts_cosyvoice import init_cosyvoice
from .synthesize.step050_synthesize_video import synthesize_all_video_under_folder
...
def initialize_models(tts_method, asr_method, diarization):
    ...
    with ThreadPoolExecutor() as executor:
        try:
            ...
            # TTS模型初始化
            # XTTS model will be loaded lazily when tts() is called with target_language
            # This allows Vietnamese XTTS model to be loaded correctly
            if tts_method == 'xtts' and not models_initialized['xtts']:
                # Skip pre-initialization for XTTS to allow language-specific model loading
                models_initialized['xtts'] = True
                logger.info("XTTS模型将在首次使用时加载")
                ...
                # Lazy load or pre-load VITS? Lazy is safer for now.
                models_initialized['vits'] = True
                logger.info("VITS model will be loaded lazily")
...
def do_everything(root_folder, url, video_file=None, num_videos=5, resolution='1080p',
                  ...
                  translation_method='LLM', translation_target_language='Japanese',
                  tts_method='xtts', tts_target_language='Japanese', voice='ja-JP-NanamiNeural',
    ...
    try:
        ...
        if video_file:
            try:
                ...
                shutil.copy(video_file, new_file_path)
                ...
                stack_trace = traceback.format_exc()
                logger.error(f"处理上传视频失败: {str(e)}\n{stack_trace}")
                return f"处理上传视频失败: {str(e)}", None
