...
def release_whisperx():
    """
    Release WhisperX and all related models (align, diarize).
    """
    global whisper_model, diarize_model, align_model, language_code, align_metadata
    logger.info("Releasing WhisperX models...")
    whisper_model = None
    diarize_model = None
    align_model = None
    language_code = None
...
def load_whisper_model(model_name: str = 'large', download_root = 'models/ASR/whisper', device='auto'):
    ...
    if device=='cpu':
        ...
        # Force float16 or int8 for GPU to save memory
        whisper_model = whisperx.load_model(model_name, download_root=download_root, device=device, compute_type='int8')
    t_end = time.time()
